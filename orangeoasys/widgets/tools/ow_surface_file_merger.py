...
from orangeoasys.util.oasys_objects import OasysPreProcessorData, OasysErrorProfileData, OasysSurfaceData

import orangeoasys.util.oasys_util as OU
...
class OWSurfaceFileMerger(OWWidget):
    ...
    class Inputs:
        data1 = Input("Surface Data #1", object)
        data2 = Input("Surface Data #2", object)
    ...
    class Outputs:
        data_preprocessor = Output("PreProcessor_Data", OasysPreProcessorData)
        data_surface = Output("Surface Data", OasysSurfaceData)
    ...
    def compute(self, plot_data=False):
        try:
            if not self.xx_1 is None and not self.xx_2 is None:
                ...
                self.Outputs.data_preprocessor.send(OasysPreProcessorData(error_profile_data=OasysErrorProfileData(surface_data=OasysSurfaceData(xx=xx,
                                                                                                                                            yy=yy,
                                                                                                                                            zz=zz,
                                                                                                                                            surface_data_file=self.surface_file_name),
                                                                                                              error_profile_x_dim=error_profile_x_dim,
                                                                                                              error_profile_y_dim=error_profile_y_dim)))
                self.Outputs.data_surface.send(OasysSurfaceData(xx=xx,
                                                           yy=yy,
                                                           zz=zz,
                                                           surface_data_file=self.surface_file_name))
...
if __name__ == "__main__":
    ...
    WidgetPreview(OWSurfaceFileMerger).run()